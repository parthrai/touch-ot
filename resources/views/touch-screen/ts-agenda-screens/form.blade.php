...
>
  ...
  <div class="form-group{{ $errors->has('type') ? ' has-error' : '' }}">
    <label for="type" class="col-xs-4 col-sm-4 col-md-4 col-lg-4 control-label">Type</label>
    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
      <select
        class="form-control"
        name="type"
      >
        @php
          $type_list = [ 'announcement', 'image', 'schedule' ];
        @endphp
        @if( isset( $screen ) )
  ...
  <div class="form-group{{ $errors->has('date') ? ' has-error' : '' }}">
    ...
    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
      <input
        ...
        type="date"
        name="date"
        value="{{ isset( $screen->date ) ? $screen->date : '' }}"
        required
      >
      @if( $errors->has( 'date' ) )
        <span class="help-block">
  ...
  <!-- BEGIN: Agenda Announcement ****************************************** -->
  <div class="form-group{{ $errors->has('agenda_announcement_id') ? ' has-error' : '' }}">
    <label for="agenda_announcement_id" class="col-xs-4 col-sm-4 col-md-4 col-lg-4 control-label">Agenda Announcement</label>
    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
      <select
        class="form-control"
        name="agenda_announcement_id"
      >
        <option value="">-- Select Agenda Announcement --</option>
        @if( isset( $screen ) )
          @foreach( $agenda_announcements as $agenda_announcement )
            @if( $screen->agenda_announcement_id == $agenda_announcement->id )
              <option
                value="{{ $agenda_announcement->id }}"
                selected="selected"
              >{{ $agenda_announcement->announcement }}</option>
            @else
              <option
                value="{{ $agenda_announcement->id }}"
              >{{ $agenda_announcement->announcement }}</option>
      ...
      @if( $errors->has( 'agenda_announcement_id' ) )
        <span class="help-block">
          <strong>{{ $errors->first( 'agenda_announcement_id' ) }}</strong>
        </span>
      @endif
    </div>
  </div>
  <!-- END: Agenda Announcement ******************************************** -->
  ...
  <div class="form-group">
    ...
    >
      <a
        ...
        href="{{ route( 'ts-agenda-screens', [ 'event_instance_name' => $event_instance_name ] ) }}"
      >Cancel</a>
    </div>
  </div>
  <!-- END: Submit ********************************************************* -->
